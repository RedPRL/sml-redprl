...
$\Member{\SemBrackets{t}_\mathcal{M}}{\LCFAtomicTactic}$.  Then, we can
interpret all of \textbf{Nominal LCF} into $\mathcal{M}$, by defining an
elaboration judgment $\LCFElabVerbose{\Upsilon}{\Gamma}{t}{T}$ with
$\Member{T}{\LCFAtomicTactic}$, such that $\Member{\rho(x)}{\LCFAtomicTactic}$
for each $\Member{x:\SortATac}{\Gamma}$.

When
it leads to no ambiguity, we will write
$\LCFElab{\Upsilon}{\Gamma}{t}{T}$ instead of the more verbose
$\LCFElabVerbose{\Upsilon}{\Gamma}{t}{T}$; we will explain this
form of judgment for $t$ of sorts $\SortATac$, $\SortBTac$, and $\SortTac$. To
start with, we will give the elaboration rules for constants (atomic tactics)
and variables denoting atomic tactics:

\[
  \infer{
    \LCFElab{\Upsilon}{\Gamma}{t}{\SemBrackets{t}_\mathcal{M}}
  }{
  }
  \qquad
  \infer{
    \LCFElab{\Upsilon}{\Gamma}{x}{\rho(x)}
  }{
  }
\]
...
\[
  \infer{
    \LCFElab{\Upsilon}{\Gamma}{\TacSmash{t_1}{t_2}}{
      \lambda\alpha.\mathtt{THEN}(T_1(\alpha),\lambda J. T_2(\StreamChop{\alpha}{\ModC{T_1}(J,\alpha)}))
    }
  }{
    \LCFElab{\Upsilon}{\Gamma}{t_1}{T_1} &
    \LCFElab{\Upsilon}{\Gamma}{t_2}{T_2}
  }
\]
...
\[
  \infer{
    ...
    \LCFElab{\Upsilon}{\Gamma}{t_1}{T_1} &
    \LCFElab{\Upsilon,\vec{u}:\SortHyp}{\Gamma}{t_2}{T_2}
  }
\]
...
\[
  \infer{
    ...
    \HypJ{
      \LCFElab{\Upsilon,\vec{u}:\SortHyp}{\Gamma}{t_i}{T_i}
    }{
      \LT{i}{n}
    }
  }
