...
struct
  ...
  local
    fun plug m = 
      fn APP (HOLE, n) => Syn.intoApp (m, n)
       ...
       | NAT_REC (motive, HOLE, zer, (x, y, succ)) => Syn.into @@ Syn.NAT_REC (motive, m, (zer, (x, y, succ)))
       | INT_REC (motive, HOLE, zer, (x,y,succ), negone, (x',y',negss)) => Syn.into @@ Syn.INT_REC (motive, m, (zer, (x,y,succ), negone, (x',y',negss)))
  ...
  fun stepView sign stability unfolding tau =
    fn `x || _ => raise Neutral (VAR x)
     ...
     | O.NAT_REC $ [[z] \ c, _ \ m, _ \ n, [x,y] \ p] || (syms, stk) => COMPAT @@ m || (syms, NAT_REC ((z, c), HOLE, n, (x,y,p)) :: stk)
     | O.ZERO $ _ || (syms, NAT_REC (_, HOLE, zer, _) :: stk) => CRITICAL @@ zer || (syms, stk)
     | O.SUCC $ [_ \ n] || (syms, NAT_REC (motive, HOLE, zer, (x,y, succ)) :: stk) =>
       let
         val rho = VarKit.ctxFromList [(n, x), (Syn.into @@ Syn.NAT_REC (motive, n, (zer, (x,y,succ))), y)]
       in
     ...
     | O.INT_REC $ [[z] \ motive, _ \ m, _ \ n, [x,y] \ p, _ \ q, [x',y'] \ r] || (syms, stk) => COMPAT @@ m || (syms, INT_REC ((z, motive), HOLE, n, (x,y,p), q, (x',y',r)) :: stk)
     | O.ZERO $ _ || (syms, INT_REC (_, HOLE, n, _, _, _) :: stk) => CRITICAL @@ n || (syms, stk)
     | O.SUCC $ [_ \ m] || (syms, INT_REC (motive, HOLE, n, (x,y,p), _, _) :: stk) =>
       let
         val rho = VarKit.ctxFromList [(m, x), (Syn.into @@ Syn.NAT_REC (motive, m, (n, (x,y,p))), y)]
       in
         CRITICAL @@ substVarenv rho p || (syms, stk)
       end
     | O.NEGSUCC $ [_ \ m] || (syms, INT_REC (motive, HOLE, _, _, q, (x,y,r)) :: stk) =>
       COMPAT @@ m || (syms, NAT_REC (motive, HOLE, q, (x,y,r)) :: stk)
