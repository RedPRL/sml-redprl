...
struct
  ...
  fun inv (G, W, m, c) = 
    case (W, m, c) of
       (_, CNEU r, _) => inv (G, W, m, c)
     | (_, RET v, UP a) => inv (G, W, m, c)
     | (_, FUN (p, m'), ARR (a, b)) => inv (G, (p, a) :: W, m', b)
     | (_, RECORD mrow, OBJ crow) => Row.app (fn (lbl, d) => inv (G, W, Row.lookup mrow lbl, d)) crow
     | ([], _, _) => invEmpty (G, m, c)
     | (p :: W', _, _) => invPat (G, W, p, m, c)
