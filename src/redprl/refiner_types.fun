...
struct
  ...
  struct
    ...
    fun EqElim alpha jdg =
      let
        ...
        val goalTy = makeEqType Hz (c0z, c1z)

        (* eliminated term *)
        val goalM = makeEq H ((m0, m1), Syn.into Syn.WBOOL)

        (* result type*)
        val goalTy0 = makeSubType H (substVar (m0, x) c0x) ty
  ...
  struct
    ...
    fun EqElim alpha jdg =
      let
        ...
        val H >> AJ.EQ ((elim0, elim1), ty) = jdg
        ...
        (* result type *)
        val goalTy = makeSubType H (substVar (m0, z) holeC) ty
    ...
    fun EqTypeElim alpha jdg =
      let
        ...
        val H >> AJ.EQ_TYPE (elim0, elim1) = jdg
        val Syn.NAT_REC (m0, (n0, (a0, b0, p0))) = Syn.out elim0
        val Syn.NAT_REC (m1, (n1, (a1, b1, p1))) = Syn.out elim1
        ...
        (* result type *)
        val goalTy = makeSubUniverse H (substVar (m0, z) holeC, raise Fail "TODO", raise Fail "TODO")
  ...
  struct
    ...
    fun Elim z alpha jdg =
      let
        ...
        val goalCoh0 = makeEqIfDifferent H ((l0, holeB), cbase) (* holeB well-typed *)
        val goalCoh1 = makeEqIfAllDifferent H ((l1, holeB), cbase) [l0]
    ...
    fun EqElim alpha jdg =
      let
        ...
        val c0z = VarKit.rename (z, x) c0x
        val c1z = VarKit.rename (z, y) c1y
        val goalC = makeEqType (H @> (z, AJ.TRUE S1)) (c0z, c1z)

        (* eliminated term *)
        val goalM = makeEq H ((m0, m1), S1)

        (* result type *)
        val goalTy = makeSubType H (substVar (m0, x) c0x) ty (* c0m0 type *)
  ...
  struct
    ...
    fun EqApp _ jdg =
      let
        ...
        val goalTy = makeSubType H holeCod ty
      in
    ...
    fun EqTypeApp _ jdg =
      let
        ...
        val goalFunEq = makeEqIfDifferent H ((m0, m1), holeFun)
        val goalArgEq = makeEq H ((n0, n1), holeDom)
        val goalTy = makeSubUniverse H (holeCod, raise Fail "TODO", raise Fail "TODO")
      in
        |>: goalFun >: goalDom >: goalCod >:? goalFunEq >: goalArgEq >: goalTy
        #> (H, trivial)
      end
  ...
  struct
    ...
    fun EqType alpha jdg =
      let
        ...
        val H >> AJ.EQ_TYPE (record0, record1) = jdg
          ...
          ListPair.foldlEq
            (fn (((lbl0, var0), ty0), ((lbl1, var1), ty1), {goals, hyps, ren0, ren1, isFirst}) =>
               let
                 ...
                 val goals' = goals >: makeEqType hyps (ty0', ty1')
    ...
    fun EqProj _ jdg =
      let
        ...
        val H >> AJ.EQ ((proj0, proj1), ty) = jdg
        ...
        val goalTy' = makeSubType H holeTyP ty
      in
  ...
  struct
    ...
    fun EqType alpha jdg =
      let
        ...
        val tyGoal = makeEqType (H @> (w, AJ.TERM O.DIM)) (a0w, a1w)
    ...
    fun True alpha jdg =
      let
        ...
        val goalCoh0 = makeEqIfDifferent H ((m0, p0), a0)
        val goalCoh1 = makeEqIfDifferent H ((m1, p1), a1)
    ...
    fun EqApp _ jdg =
      let
        ...
        val goalTy = makeSubType H holeLine ty (* holeLine type *)
      in
    ...
    fun EqAppConst _ jdg =
      let
        ...
        val goalTy = makeSubType H holeLine a (* holeLine should be well-typed *)
  ...
  struct
    ...
    fun EqType alpha jdg =
      let
        ...
        val goalA = makeEqType H (a0, a1)
        val goalB = makeEqType H (b0, b1)
        val goalC = makeEqType H (c0, c1)
    ...
    fun EqGlue alpha jdg =
      let
        ...
        val goalCohF = makeEq H ((substVar (m0, x) fx, fm0), a)
        val goalCohG = makeEq H ((substVar (m0, y) gy, gm0), b)
      in
    ...
    fun EqElim alpha jdg =
      let
        ...
        val goalC = makeEqType (H @> (z, AJ.TRUE holeTyPushout)) (d0z, d1z)

        (* eliminated term *)
        val goalM = makeEqIfDifferent H ((m0, m1), holeTyPushout)

        (* result type*)
        val goalTy0 = makeSubType H (substVar (m0, z0) d0z0) ty
  ...
  struct
    ...
    fun NondetTypeFromTrueEqAtType z _ jdg =
      let
        ...
        val goal = makeType H ty0 (* what is this premise for ?? *)
      in
        |>: goal #> (H, trivial)
      end
  ...
  struct
    ...
    local
      fun genTubeGoals' H (tubes0, tubes1) =
        ListPairUtil.mapPartialEq
          (fn ((eq, t0), (_, t1)) => Restriction.makeEqType [eq] H (t0, t1))
          (tubes0, tubes1)
      fun genInterTubeGoalsExceptDiag' H (tubes0, tubes1) =
        ComKit.enumInterExceptDiag
          (fn ((eq0, t0), (eq1, t1)) => Restriction.makeEqTypeIfDifferent [eq0, eq1] H (t0, t1))
          (tubes0, tubes1)
    in
      fun genInterTubeGoals H w (tubes0, tubes1) =
        let
          val tubes0 = ComKit.alphaRenameTubes w tubes0
          val tubes1 = ComKit.alphaRenameTubes w tubes1

          val goalsOnDiag = genTubeGoals' (H @> (w, AJ.TERM O.DIM)) (tubes0, tubes1)
          val goalsNotOnDiag = genInterTubeGoalsExceptDiag' (H @> (w, AJ.TERM O.DIM)) (tubes0, tubes1)
        in
    ...
    (* see the function of th same name in `ComKit` *)
    fun genCapTubeGoalsIfDifferent H (cap, (r, tubes)) =
      List.mapPartial
        (fn (eq, (u, tube)) =>
          Restriction.makeEqTypeIfDifferent [eq] H (cap, substVar (r, u) tube))
        tubes
    ...
    fun EqType alpha jdg =
      let
        ...
        val goalCap = makeEqType H (cap0, cap1)

        val w = alpha 0
      in
        |>: goalCap
         >:+ genInterTubeGoals H w (tubes0, tubes1)
         >:+ genCapTubeGoalsIfDifferent H (cap0, (#1 dir0, tubes0))
        #> (H, trivial)
      end
    ...
    fun Eq alpha jdg =
      ...
      in
        |>: goalCap
         ...
         >:+ genInterTubeGoals H w (tyTubes, tyTubes)
         >:+ genCapTubeGoalsIfDifferent H (tyCap, (#1 dir, tyTubes))
        #> (H, trivial)
      end
  ...
  struct
    ...
    fun EqType _ jdg =
      let
        ...
        val goalA = Restriction.makeEqType [eq] H (a0, a1)
        val goalB = makeEqType H (b0, b1)
  ...
  struct
    ...
    (* To be deleted once we delete EQ_TYPE *)
    fun Eq _ jdg =
      let
        val _ = RedPrlLog.trace "Universe.Eq"
        val H >> AJ.EQ ((a, b), ty) = jdg
    ...
    fun ElimFromTrue z alpha jdg =
      let
        ...
        val (goal, hole) =
          makeGoal
            @@ (Hyps.interposeAfter (z, |@> (u, AJ.TYPE (VarKit.toExp z))) H)
            >> catjdg
      in
        |>: goal #> (H, VarKit.subst (trivial, u) hole)
      end
    ...
    fun ElimFromEq z alpha jdg =
      let
        ...
        val H >> catjdg = jdg
        (* for now we ignore the kind and the level in the context *)
        val AJ.EQ ((ty1, ty2), univ) = Hyps.lookup H z
        val Syn.UNIVERSE (l, k) = Syn.out univ

        val u = alpha 0
        val (goal, hole) =
          makeGoal
            @@ (Hyps.interposeAfter (z, |@> (u, AJ.EQ_TYPE (ty1, ty2))) H)
            >> catjdg
      in
        |>: goal #> (H, VarKit.subst (trivial, u) hole)
      end
    ...
    fun NondetSynthFromEqType z _ jdg =
      let
        ...
        val H >> AJ.SYNTH ty2 = jdg
        val AJ.EQ_TYPE (ty0, ty1) = Hyps.lookup H z
